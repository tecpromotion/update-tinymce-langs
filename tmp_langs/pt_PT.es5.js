tinymce.addI18n('pt-PT', {
  "#": "#",
  "Accessibility": "Acessibilidade",
  "Accordion": "",
  "Accordion body...": "",
  "Accordion summary...": "",
  "Action": "Ação",
  "Activity": "Atividades",
  "Address": "Endereço",
  "Advanced": "Avançado",
  "Align": "Alinhar",
  "Align center": "Alinhar ao centro",
  "Align left": "Alinhar à esquerda",
  "Align right": "Alinhar à direita",
  "Alignment": "Alinhamento",
  "Alignment {0}": "",
  "All": "Tudo",
  "Alternative description": "Descrição alternativa",
  "Alternative source": "Origem alternativa",
  "Alternative source URL": "URL da origem alternativa",
  "Anchor": "Âncora",
  "Anchor...": "Âncora...",
  "Anchors": "Âncoras",
  "Animals and Nature": "Animais e natureza",
  "Arrows": "Setas",
  "B": "B",
  "Background color": "Cor de fundo",
  "Background color {0}": "",
  "Black": "Preto",
  "Block": "Bloco",
  "Block {0}": "",
  "Blockquote": "Citação em bloco",
  "Blocks": "Blocos",
  "Blue": "Azul",
  "Blue component": "Componente azul(Blue)",
  "Body": "Corpo",
  "Bold": "Negrito",
  "Border": "Contorno",
  "Border color": "Cor do contorno",
  "Border style": "Estilo do contorno",
  "Border width": "Largura do contorno",
  "Bottom": "Inferior",
  "Browse files": "",
  "Browse for an image": "Navegar por uma imagem",
  "Browse links": "",
  "Bullet list": "Lista com marcas",
  "Cancel": "Cancelar",
  "Caption": "Legenda",
  "Cell": "Célula",
  "Cell padding": "Preenchimento da célula",
  "Cell properties": "Propriedades da célula",
  "Cell spacing": "Espaçamento entre células",
  "Cell styles": "Estilo de células",
  "Cell type": "Tipo de célula",
  "Center": "Centro",
  "Characters": "Caracteres",
  "Characters (no spaces)": "Caracteres (sem espaços)",
  "Circle": "Círculo",
  "Class": "Classe",
  "Clear formatting": "Limpar formatação",
  "Close": "Fechar",
  "Code": "Código",
  "Code sample...": "Amostra de código...",
  "Code view": "Vista do código-fonte",
  "Color Picker": "Seletor de cores",
  "Color swatch": "Coleção de cores",
  "Cols": "Colunas",
  "Column": "Coluna",
  "Column clipboard actions": "Ações da coluna da área de transferência",
  "Column group": "Grupo de colunas",
  "Column header": "Cabeçalho de coluna",
  "Constrain proportions": "Manter proporções",
  "Copy": "Copiar",
  "Copy column": "Copiar coluna",
  "Copy row": "Copiar linha",
  "Could not find the specified string.": "Não foi possível localizar a cadeia de caracteres especificada.",
  "Could not load emojis": "Não conseguiu carregar os emojis",
  "Count": "Contagem",
  "Currency": "Moeda",
  "Current window": "Janela atual",
  "Custom color": "Cor personalizada",
  "Custom...": "Personalizado...",
  "Cut": "Cortar",
  "Cut column": "Cortar coluna",
  "Cut row": "Cortar linha",
  "Dark Blue": "Azul escuro",
  "Dark Gray": "Cinzento escuro",
  "Dark Green": "Verde escuro",
  "Dark Orange": "Laranja escuro",
  "Dark Purple": "Púrpura escuro",
  "Dark Red": "Vermelho escuro",
  "Dark Turquoise": "Turquesa escuro",
  "Dark Yellow": "Amarelo escuro",
  "Dashed": "Tracejada",
  "Date/time": "Data/hora",
  "Decrease indent": "Diminuir avanço",
  "Default": "Predefinição",
  "Delete accordion": "",
  "Delete column": "Eliminar coluna",
  "Delete row": "Eliminar linha",
  "Delete table": "Eliminar tabela",
  "Dimensions": "Dimensões",
  "Disc": "Disco",
  "Div": "Div",
  "Document": "Documento",
  "Dotted": "Ponteada",
  "Double": "Duplo",
  "Drop an image here": "Largar aqui uma imagem",
  "Dropped file type is not supported": "O tipo de ficheiro largado não é suportado",
  "Edit": "Editar",
  "Embed": "Incorporar",
  "Emojis": "Emojis",
  "Emojis...": "Emojis...",
  "Error": "Erro",
  "Error: Form submit field collision.": "Erro: conflito no campo de submissão de formulário.",
  "Error: No form element found.": "Erro: nenhum elemento de formulário encontrado.",
  "Extended Latin": "Latino estendido",
  "Failed to initialize plugin: {0}": "Falha ao inicializar o plugin: {0}",
  "Failed to load plugin url: {0}": "Falha ao carregar o URL do plugin: {0}",
  "Failed to load plugin: {0} from url {1}": "Falha ao carregar o plugin: {0} do URL {1}",
  "Failed to upload image: {0}": "Falha ao carregar a imagem: {0}",
  "File": "Ficheiro",
  "Find": "Localizar",
  "Find (if searchreplace plugin activated)": "Pesquisar (se o plugin localizar e substituir estiver ativado)",
  "Find and Replace": "Pesquisar e substituir",
  "Find and replace...": "Localizar e substituir...",
  "Find in selection": "Pesquisar na seleção",
  "Find whole words only": "Localizar apenas palavras inteiras",
  "Flags": "Bandeiras",
  "Focus to contextual toolbar": "Foco na barra de ferramentas contextual",
  "Focus to element path": "Foco no caminho do elemento",
  "Focus to menubar": "Foco na barra de menu",
  "Focus to toolbar": "Foco na barra de ferramentas",
  "Font": "Tipo de letra",
  "Font size {0}": "",
  "Font sizes": "Tamanhos de letra",
  "Font {0}": "",
  "Fonts": "Tipos de letra",
  "Food and Drink": "Comida e bebida",
  "Footer": "Rodapé",
  "Format": "Formatar",
  "Format {0}": "",
  "Formats": "Formatos",
  "Fullscreen": "Ecrã inteiro",
  "G": "G",
  "General": "Geral",
  "Gray": "Cinzento",
  "Green": "Verde",
  "Green component": "Componente verde(Green)",
  "Groove": "Entalhado",
  "Handy Shortcuts": "Atalhos úteis",
  "Header": "Cabeçalho",
  "Header cell": "Célula de cabeçalho",
  "Heading 1": "Título 1",
  "Heading 2": "Título 2",
  "Heading 3": "Título 3",
  "Heading 4": "Título 4",
  "Heading 5": "Título 5",
  "Heading 6": "Título 6",
  "Headings": "Títulos",
  "Height": "Altura",
  "Help": "Ajuda",
  "Hex color code": "Código de cor hexadécimal",
  "Hidden": "Escondido",
  "Horizontal align": "Alinhamento horizontal",
  "Horizontal line": "Linha horizontal",
  "Horizontal space": "Espaço horizontal",
  "ID": "ID",
  "ID should start with a letter, followed only by letters, numbers, dashes, dots, colons or underscores.": "O ID deve começar com uma letra, seguido apenas por letras, números, traços, pontos, dois pontos ou sobtraços.",
  "Image is decorative": "Imagem é decorativa",
  "Image list": "Lista de imagens",
  "Image title": "Título da imagem",
  "Image...": "Imagem...",
  "ImageProxy HTTP error: Could not find Image Proxy": "Erro de ImageProxy HTTP: Proxy de Imagem não encontrado",
  "ImageProxy HTTP error: Incorrect Image Proxy URL": "Erro de ImageProxy HTTP: URL do Proxy de Imagem incorreto",
  "ImageProxy HTTP error: Rejected request": "Erro de ImageProxy HTTP: Pedido regeitado",
  "ImageProxy HTTP error: Unknown ImageProxy error": "Erro de ImageProxy HTTP: Erro de Proxy de Imagem desconhecido",
  "Increase indent": "Aumentar avanço",
  "Inline": "Na linha",
  "Insert": "Inserir",
  "Insert Template": "Inserir modelo",
  "Insert accordion": "",
  "Insert column after": "Inserir coluna depois",
  "Insert column before": "Inserir coluna antes",
  "Insert date/time": "Inserir data/hora",
  "Insert image": "Inserir imagem",
  "Insert link (if link plugin activated)": "Inserir hiperligação (se o plugin de ligações estiver ativado)",
  "Insert row after": "Inserir linha depois",
  "Insert row before": "Inserir linha antes",
  "Insert table": "Inserir tabela",
  "Insert template...": "Inserir modelo...",
  "Insert video": "Inserir vídeo",
  "Insert/Edit code sample": "Inserir/editar amostra de código",
  "Insert/edit image": "Inserir/editar imagem",
  "Insert/edit link": "Inserir/editar ligação",
  "Insert/edit media": "Inserir/editar media",
  "Insert/edit video": "Inserir/editar vídeo",
  "Inset": "Inserido",
  "Invalid hex color code: {0}": "Código de cor hexadécimal inválido: {0}",
  "Invalid input": "Entrada inválida",
  "Italic": "Itálico",
  "Justify": "Justificar",
  "Keyboard Navigation": "Navegação com teclado",
  "Language": "Idioma",
  "Learn more...": "Saiba mais...",
  "Left": "Esquerda",
  "Left to right": "Da esquerda para a direita",
  "Light Blue": "Azul claro",
  "Light Gray": "Cinzento claro",
  "Light Green": "Verde claro",
  "Light Purple": "Púrpura claro",
  "Light Red": "Vermelho claro",
  "Light Yellow": "Amarelo claro",
  "Line height": "Altura da linha",
  "Link list": "Lista de ligações",
  "Link...": "Ligação...",
  "List Properties": "Propriedades da lista",
  "List properties...": "Propriedades da lista…",
  "Loading emojis...": "A carregar emojis...",
  "Loading...": "A carregar...",
  "Lower Alpha": "Latino em minúsculas",
  "Lower Greek": "Grego em minúsculas",
  "Lower Roman": "Romano em minúsculas",
  "Match case": "Diferenciar maiúsculas/minúsculas",
  "Mathematical": "Matemático",
  "Media poster (Image URL)": "Publicador de media (URL da imagem)",
  "Media...": "Media...",
  "Medium Blue": "Azul intermédio",
  "Medium Gray": "Cinzento médio",
  "Medium Purple": "Púrpura intermédio",
  "Merge cells": "Fundir células",
  "Middle": "Meio",
  "Midnight Blue": "Azul muito escuro",
  "More...": "Mais...",
  "Name": "Nome",
  "Navy Blue": "Azul-atlântico",
  "New document": "Novo documento",
  "New window": "Nova janela",
  "Next": "Seguinte",
  "No": "Não",
  "No alignment": "Sem alinhamento",
  "No color": "Sem cor",
  "Nonbreaking space": "Espaço não separável",
  "None": "Nenhum",
  "Numbered list": "Lista numerada",
  "OR": "OU",
  "Objects": "Objetos",
  "Ok": "Ok",
  "Open help dialog": "Abrir caixa de diálogo Ajuda",
  "Open link": "Abrir ligação",
  "Open link in...": "Abrir ligação em...",
  "Open popup menu for split buttons": "Abrir o menu popup para botões divididos",
  "Orange": "Laranja",
  "Outset": "Extraído",
  "Page break": "Quebra de página",
  "Paragraph": "Parágrafo",
  "Paste": "Colar",
  "Paste as text": "Colar como texto",
  "Paste column after": "Colar coluna depois",
  "Paste column before": "Colar coluna antes",
  "Paste is now in plain text mode. Contents will now be pasted as plain text until you toggle this option off.": "O comando colar está em modo de texto simples. O conteúdo será colado como texto simples até desativar esta opção.",
  "Paste or type a link": "Copiar ou escrever uma hiperligação",
  "Paste row after": "Colar linha depois",
  "Paste row before": "Colar linha antes",
  "Paste your embed code below:": "Insira, abaixo, o seu código de incorporação:",
  "People": "Pessoas",
  "Plugins": "Plugins",
  "Plugins installed ({0}):": "Plugins instalados ({0}):",
  "Powered by {0}": "Com tecnologia {0}",
  "Pre": "Pre",
  "Preferences": "Preferências",
  "Preformatted": "Pré-formatado",
  "Premium plugins:": "Plugins comerciais:",
  "Press the Up and Down arrow keys to resize the editor.": "",
  "Press the arrow keys to resize the editor.": "",
  "Press {0} for help": "",
  "Preview": "Pré-visualizar",
  "Previous": "Anterior",
  "Print": "Imprimir",
  "Print...": "Imprimir...",
  "Purple": "Púrpura",
  "Quotations": "Aspas",
  "R": "R",
  "Range 0 to 255": "Intervalo de 0 a 255",
  "Red": "Vermelho",
  "Red component": "Componente vermelho(Red)",
  "Redo": "Refazer",
  "Remove": "Remover",
  "Remove color": "Remover cor",
  "Remove link": "Remover ligação",
  "Replace": "Substituir",
  "Replace all": "Substituir tudo",
  "Replace with": "Substituir por",
  "Resize": "Redimensionar",
  "Restore last draft": "Restaurar o último rascunho",
  "Reveal or hide additional toolbar items": "",
  "Rich Text Area": "Área de Rich Text",
  "Rich Text Area. Press ALT-0 for help.": "Área de texto formatado. Prima ALT-0 para exibir a ajuda.",
  "Rich Text Area. Press ALT-F9 for menu. Press ALT-F10 for toolbar. Press ALT-0 for help": "Área de texto formatado. Prima ALT-F9 para exibir o menu. Prima ALT-F10 para exibir a barra de ferramentas. Prima ALT-0 para exibir a ajuda",
  "Ridge": "Crista",
  "Right": "Direita",
  "Right to left": "Da direita para a esquerda",
  "Row": "Linha",
  "Row clipboard actions": "Ações da linha da área de transferência",
  "Row group": "Grupo de linhas",
  "Row header": "Cabeçalho de linha",
  "Row properties": "Propriedades da linha",
  "Row type": "Tipo de linha",
  "Rows": "Linhas",
  "Save": "Guardar",
  "Save (if save plugin activated)": "Guardar (se o plugin de guardar estiver ativado)",
  "Scope": "Âmbito",
  "Search": "Pesquisar",
  "Select all": "Selecionar tudo",
  "Select...": "Selecionar...",
  "Selection": "Seleção",
  "Shortcut": "Atalho",
  "Show blocks": "Mostrar blocos",
  "Show caption": "Mostrar legenda",
  "Show invisible characters": "Mostrar carateres invisíveis",
  "Size": "Tamanho",
  "Solid": "Sólida",
  "Source": "Origem",
  "Source code": "Código fonte",
  "Special Character": "Carácter especial",
  "Special character...": "Carácter especial...",
  "Split cell": "Dividir célula",
  "Square": "Quadrado",
  "Start list at number": "Começar a lista pelo número",
  "Strikethrough": "Rasurado",
  "Style": "Estilo",
  "Subscript": "Inferior à linha",
  "Superscript": "Superior à linha",
  "Switch to or from fullscreen mode": "Entrar ou sair do modo de ecrã inteiro",
  "Symbols": "Símbolos",
  "System Font": "Tipo de letra do sistema",
  "Table": "Tabela",
  "Table caption": "Legenda da tabela",
  "Table properties": "Propriedades da tabela",
  "Table styles": "Estilos de tabelas ",
  "Template": "Modelo",
  "Templates": "Modelos",
  "Text": "Texto",
  "Text color": "Cor do texto",
  "Text color {0}": "",
  "Text to display": "Texto a mostrar",
  "The URL you entered seems to be an email address. Do you want to add the required mailto: prefix?": "O URL que indicou parece ser um endereço de email. Quer adicionar o prefixo mailto: tal como necessário?",
  "The URL you entered seems to be an external link. Do you want to add the required http:// prefix?": "O URL que indicou parece ser uma ligação externa. Deseja adicionar o prefixo http:// necessário?",
  "The URL you entered seems to be an external link. Do you want to add the required https:// prefix?": "O URL que introduziu parece ser uma ligação externa. Deseja adicionar-lhe o prefixo https:// ?",
  "Title": "Título",
  "To open the popup, press Shift+Enter": "Para abrir o pop-up, prima Shift+Enter",
  "Toggle accordion": "",
  "Tools": "Ferramentas",
  "Top": "Superior",
  "Travel and Places": "Viagens e lugares",
  "Turquoise": "Turquesa",
  "Underline": "Sublinhado",
  "Undo": "Desfazer",
  "Upload": "Carregar",
  "Uploading image": "A carregar imagem",
  "Upper Alpha": "Latino em maiúsculas",
  "Upper Roman": "Romano em maiúsculas",
  "Url": "URL",
  "User Defined": "Definido pelo utilizador",
  "Valid": "Válido",
  "Version": "Versão",
  "Vertical align": "Alinhamento vertical",
  "Vertical space": "Espaço vertical",
  "View": "Ver",
  "Visual aids": "Ajuda visual",
  "Warn": "Aviso",
  "White": "Branco",
  "Width": "Largura",
  "Word count": "Contagem de palavras",
  "Words": "Palavras",
  "Words: {0}": "Palavras: {0}",
  "Yellow": "Amarelo",
  "Yes": "Sim",
  "You are using {0}": "Está a usar {0}",
  "You have unsaved changes are you sure you want to navigate away?": "Existem alterações que ainda não foram guardadas. Tem a certeza que pretende sair?",
  "Your browser doesn't support direct access to the clipboard. Please use the Ctrl+X/C/V keyboard shortcuts instead.": "O seu navegador não suporta acesso direto à área de transferência. Por favor, use os atalhos Ctrl+X/C/V do seu teclado.",
  "alignment": "alinhamento",
  "austral sign": "Austral - símbolo de moeda",
  "cedi sign": "Cedi - símbolo de moeda",
  "colon sign": "sinal de dois pontos",
  "cruzeiro sign": "Cruzeiro - símbolo de moeda",
  "currency sign": "símbolo de moeda",
  "dollar sign": "Dólar - símbolo de moeda",
  "dong sign": "Dong - símbolo de moeda",
  "drachma sign": "Dracma - símbolo de moeda",
  "euro-currency sign": "Euro - símbolo de moeda",
  "example": "exemplo",
  "formatting": "formatação",
  "french franc sign": "Franco francês - símbolo de moeda",
  "german penny symbol": "Penny alemão - símbolo de moeda",
  "guarani sign": "Guarani - símbolo de moeda",
  "history": "histórico",
  "hryvnia sign": "Grívnia - símbolo de moeda",
  "indentation": "indentação",
  "indian rupee sign": "Rupia indiana - símbolo de moeda",
  "kip sign": "Kip - símbolo de moeda",
  "lira sign": "Lira - símbolo de moeda",
  "livre tournois sign": "Libra de Tours - símbolo de moeda",
  "manat sign": "Manat - símbolo de moeda",
  "mill sign": "sinal de por mil",
  "naira sign": "Naira - símbolo de moeda",
  "new sheqel sign": "Shekel - símbolo de moeda",
  "nordic mark sign": "Markka nórdica - símbolo de moeda",
  "peseta sign": "Peseta - símbolo de moeda",
  "peso sign": "Peso - símbolo de moeda",
  "ruble sign": "Rublo - símbolo de moeda",
  "rupee sign": "Rúpia - símbolo de moeda",
  "spesmilo sign": "Spesmilo - símbolo de moeda",
  "styles": "estilos",
  "tenge sign": "Tengue - símbolo de moeda",
  "tugrik sign": "Tugrik - símbolo de moeda",
  "turkish lira sign": "Lira turca - símbolo de moeda",
  "won sign": "Won - símbolo de moeda",
  "yen character": "Iene - símbolo de moeda",
  "yen/yuan character variant one": "Iene/Iuane - variante do símbolo de moeda",
  "yuan character": "Iuane - símbolo de moeda",
  "yuan character, in hong kong and taiwan": "Iuane, em Hong Kong e República da China - símbolo de moeda",
  "{0} characters": "{0} caracteres",
  "{0} columns, {1} rows": "",
  "{0} words": "{0} palavras"
});
